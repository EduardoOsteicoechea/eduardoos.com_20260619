...
4. **Drill-down flow (mandatory):**
   1. Liber tabs (I–IV).
   2. **Chapter number** chip grid for Capita in that Liber (PRELIMINARY as its own chip). Selecting a Caput loads that chapter JSON.
   3. **Paragraph number** chip grid appears **only after** a Caput is selected.
   4. **Text** for the **selected paragraph only** appears **below** (plain text; **no** Copy / Copy chapter buttons in this panel).
5. Backdrop click / Cerrar closes the panel.
6. Non-goal: paste-into-layer tools; clipboard buttons are out of this panel.
