...
5. **FAQ section chrome:** the outer `#faq` block uses **`home-profile__block--flat`** — **no** section background and **no** inner padding (cards sit on the page surface). Heading + stacked cards only.
...
- [x] FAQ outer section has no background and no inner padding (`--flat`)
