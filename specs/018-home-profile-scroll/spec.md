...
3. **Dossier data-section gap** — `.home-profile__grid` gap between Profile, Specialization, and other dossier blocks is **`calc(var(--m4) * 2)` (4.5rem)** (was `--m4` / 2.25rem). Agent↔dossier gutter may still use `--home-gap` per 047.
...
- [x] Dossier `.home-profile__grid` gap is `calc(var(--m4) * 2)` (4.5rem; 2× prior `--m4`).
