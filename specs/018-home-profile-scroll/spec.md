...
3. **Dossier data-section gap** — `.home-profile__grid` gap between Profile, Specialization, and other dossier blocks is **`calc(var(--m4) * 3)` (6.75rem)** (was `* 2` / 4.5rem). Agent↔dossier gutter may still use `--home-gap` per 047.
...
- [x] Dossier `.home-profile__grid` gap is `calc(var(--m4) * 3)` (6.75rem; +1× `--m4` over prior `* 2`).
