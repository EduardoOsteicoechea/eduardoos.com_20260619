...
10. **Scene class vs shadows (option A):**
    ...
    - **Shadows on:** Switch `world.scene` to That Open **`ShadowedScene`** (migrate non-light scene children so loaded models/grid survive), enable `SimpleRenderer` shadow maps (`shadowMap.enabled`, prefer `VSMShadowMap`), call `setup` with cascade `1` + resolution, `autoBias = false`, user bias, `shadowsEnabled = true`, then `updateShadows()` after setup, model load/tile flags, sun changes, and camera controls `rest`. Opaque fragment tiles set `castShadow` / `receiveShadow`. Add a large Y=0 `ShadowMaterial` ground plane that receives shadows. Turning shadows **off** switches back to **`SimpleScene`** (plain `setup()` then re-apply the user’s current ambient/directional/sun settings so sliders stay live).
...
- [x] Sidebar nominal defaults document ambient `1` `#ffffff`, directional `1.5` `#ffffff`, sun from `(5, 10, 3)`; enabling Shadows switches to ShadowedScene + ground + shadow maps.
...
- [x] Lights sidebar includes **Shadows** (enable, map resolution, bias); enabling Shadows switches to `ShadowedScene` + renderer shadow maps + ground receiver; loaded model meshes cast/receive; turning Shadows off restores `SimpleScene`.
