...
- [x] With Shadows on and an opaque multi-slab IFC (e.g. terrain / street / sidewalk): **visible projected cast shadows** appear from one fragment mesh onto sibling fragment meshes and onto the shadow ground receiver (not flat unlit slabs). DistanceRenderer must exclude the grid and the shadow ground so CSM frustum fits the model (That Open ShadowedScene recipe).
...
- [x] With Shadows on: opaque fragment tiles cast onto sibling fragment meshes and the shadow ground (visible projected shadows). Grid + shadow ground are excluded from ShadowedScene `distanceRenderer` so CSM bounds track the model (That Open recipe).
