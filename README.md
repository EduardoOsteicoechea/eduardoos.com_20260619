...
Authenticated pamphlet PDF: `POST /api/documents/pamphlet/pdf` (JWT) → `documents` service `POST /pamphlet`. Body is the `.epam` pamphlet JSON; response is a two-page US Letter landscape PDF (279.4 × 215.9 mm, column width 57.85 mm, center gutter 20 mm). Header geometry is copied from the editor CSS: 20 mm band + 5 mm gutter, title 5 mm / line-height 1.1, meta 2.5 mm / 1.2, wrap via Helvetica-Bold AFM widths so a two-line desktop title fills the same band (no extra empty margin). Column stacking matches the sheet: item-top cursor, heading 4.25 mm / line-height 1.2, only 2.5 mm item gaps (no extra heading margin), and last lines paint when their baseline is still above the band floor so the bottom heading/paragraph is not clipped. Built by `pkg/pdf.BuildPamphletPDF` (no external PDF libs). Imprimir on the editor downloads that attachment; browser `window.print()` remains commented as fallback.
