...
// (WinAnsi / Latin-1). A flat 0.50em average let the page-1 title stay on one
// line while the desktop sheet (system-ui 5mm bold) wraps to two — leftover
// empty header band then looked like an extra margin under Autor/Fecha.
...
func init() {
	...
	overlayLatin1(&helveticaWidths)
	overlayLatin1(&helveticaBoldWidths)
}

func overlayLatin1(w *[256]int) {
	space := w[32]
	w[0xA0] = space // nbsp
	w[0xA1] = w['!']
	w[0xBF] = w['?']
