...
func ContentDispositionAttachment(rawName string) string {
	...
	// PathEscape leaves some chars; RFC 5987 wants percent-encoding of UTF-8 octets.
...
func sanitizeDownloadBase(name string) string {
	...
	name = path.Base(name) // drop any accidental path segments
	...
	// Cap by runes, not bytes, so we never split a multi-byte character.
	const maxRunes = 80
...
func asciiFilenameFallback(name string) string {
	...
	for _, r := range name {
		...
		case r == ' ':
			b.WriteByte('_')
		default:
			// Fold common Spanish glyphs to ASCII so legacy filename= stays readable.
