...
func (s *Server) RegisterRoutes(r chi.Router) {
	...
	r.Post("/absolute/multipart", s.handleUploadAbsoluteMultipart)
	r.Get("/absolute/*", s.handleGetAbsoluteFile)
}
...
func (s *Server) handleUploadAbsoluteMultipart(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseMultipartForm(32 << 20); err != nil {
		common.WriteError(w, http.StatusBadRequest, "invalid multipart form")
		return
	}
	objectKey := strings.TrimSpace(r.FormValue("absolute_key"))
	if objectKey == "" {
		common.WriteError(w, http.StatusBadRequest, "absolute_key required")
		return
	}
	file, header, err := r.FormFile("file")
	if err != nil {
	...
	_, ct, prepErr := PrepareUpload(filename, data)
	if prepErr != nil {
		common.WriteError(w, http.StatusBadRequest, prepErr.Error())
		return
	}
	result, err := s.Store.PutAbsolute(r.Context(), objectKey, ct, data)
	if err != nil {
		common.WriteError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (s *Server) handleGetAbsoluteFile(w http.ResponseWriter, r *http.Request) {
	objectKey := strings.TrimPrefix(chi.URLParam(r, "*"), "/")
	if objectKey == "" {
		common.WriteError(w, http.StatusBadRequest, "key required")
		return
	}
	data, ct, err := s.Store.GetAbsolute(r.Context(), objectKey)
	if err != nil {
		common.WriteError(w, http.StatusNotFound, "not found")
		return
	}
	if ct == "" || ct == "application/octet-stream" {
		ct = ContentTypeFromKey(objectKey)
	}
	w.Header().Set("Content-Type", ct)
	w.Header().Set("Content-Length", strconv.Itoa(len(data)))
	w.Header().Set("Cache-Control", "public, max-age=3600")
