...
import { PAMPHLET_V3_ZONE_CAPACITY_MM, pxToMm, zoneOccupationPercent, zoneUsedHeightMm, type PamphletV3ContentItem, type PamphletV3Stream, } from "./pamphletV3Content";
...
export function PamphletContentItemsContainer({ type, label, gridArea, capacityKey, content, editingItemId, itemGapMm = 0, occupationPercent, usedMm: usedMmProp, capacityMm: capacityMmProp, handlers, editDisabled = false, measureEnabled = true, onEmptyActivate, onCapacityChange, }: PamphletContentItemsContainerProps) {
    const stackRef = useRef<HTMLDivElement>(null);
    const capacityProbeRef = useRef<HTMLDivElement>(null);
    const stream = streamForContainer(type);
    const fallbackCapacityMm = type === "header"
    ...
    const localOccupation = useMemo(() => zoneOccupationPercent(usedMm, capacityMm), [capacityMm, usedMm]);
    ...
    return (<section className={`pamphlet_content_container ${classByType[type]} ${areaClass}`.trim()} data-zone-type={type} data-capacity-key={capacityKey} data-occupation-percent={percent.toFixed(1)} data-capacity-mm={capacityMm.toFixed(2)} data-used-mm={usedMm.toFixed(2)} aria-label={label || undefined} style={gridArea ? { gridArea } : undefined}>
      <div ref={stackRef} className="pamphlet_zone_stack">
        <div ref={capacityProbeRef} className="pamphlet_zone_capacity_probe" aria-hidden="true" data-capacity-probe="true"/>
        {content.length === 0 ? (onEmptyActivate ? (<button type="button" className="pamphlet_zone_empty pamphlet-no-print" onClick={onEmptyActivate}>
              Click to add content
            </button>) : null) : (content.map((item) => (<PamphletContentItem key={item.id} item={item} editing={editingItemId === item.id} editDisabled={editDisabled} measureEnabled={measureEnabled} stream={stream} handlers={handlers}/>)))}
      </div>
    </section>);
}
