...
import { pamphletV3ItemBottomMarginMm, pamphletV3ItemTopMarginMm, pxToMm, type PamphletV3ContentItem, type PamphletV3ItemType, type PamphletV3Stream, } from "./pamphletV3Content";
...
export default function PamphletContentItem({ item, editing, editDisabled, stream = "body", measureEnabled = true, handlers, }: PamphletContentItemProps) {
    ...
    useLayoutEffect(() => {
        ...
        function publishHeight() {
            ...
            const nextMm = pxToMm(root.offsetHeight) +
    ...
    return (<div ref={rootRef} className={`pamphlet-v3-item${editing ? " is-editing" : ""}${editDisabled ? " is-disabled" : ""}`} data-item-id={item.id} data-item-type={item.type} data-editing={editing ? "true" : "false"} data-height-mm={item.heightMm.toFixed(2)} style={style} onClick={handleSelect} role="presentation">
