import { PAMPHLET_V3_COLUMN_WIDTH_MM, PAMPHLET_V3_DEFAULT_COLUMN_CAPACITIES, PAMPHLET_V3_ZONE_CAPACITY_MM, exportZone, packItemsIntoZones, pamphletV3ItemHasContent, zoneOccupationPercent, zoneUsedHeightMm, type PamphletV3ColumnZoneId, type PamphletV3ContentItem, type PamphletV3ContentJson, type PamphletV3Document, type PamphletV3PackZone, } from "./pamphletV3Content";
...
export type PamphletMeasuredCapacities = Partial<Record<"header" | "footer" | PamphletV3ColumnZoneId, number>>;
function occupationFor(items: PamphletV3ContentItem[], capacityMm: number, gapMm: number, zone: "header" | "body" | "footer" = "body"): PamphletZoneOccupation {
...
function resolveCapacity(zoneId: PamphletV3ColumnZoneId | "header" | "footer", measured: PamphletMeasuredCapacities | undefined): number {
...
export default function contentDistribution(document: PamphletV3Document, measuredCapacities?: PamphletMeasuredCapacities): PamphletContentDistribution {
    ...
    const packed = packItemsIntoZones(document.bodyItems.filter(pamphletV3ItemHasContent), packZones, gapMm, PAMPHLET_V3_COLUMN_WIDTH_MM);
    const columns = {
        first: packed.first ?? [],
...
export function distributionToContentJson(distribution: PamphletContentDistribution): PamphletV3ContentJson {
    return {
