...
export default function PamphletItemView({ item, className }: PamphletItemViewProps) {
    ...
    let body: ReactNode;
    if (item.type === "list") {
        body = (<div className="pamphlet-v3-item__list">
        {item.text.trim() ? <p className="pamphlet-v3-item__list-header-view">{item.text}</p> : null}
        <ul>
          {(item.listItems.length > 0 ? item.listItems : [{ id: "empty", text: "" }]).map((row) => (<li key={row.id}>{row.text || "\u00a0"}</li>))}
        </ul>
      </div>);
    }
    else if (item.type === "image") {
        body = (<div className="pamphlet-v3-item__image">
        ...
        body = !pamphletV3ItemHasContent(item) ? (<p className={textClass}>{"\u00a0"}</p>) : (<p className={textClass} dangerouslySetInnerHTML={{ __html: item.text.trim() ? item.text : "&nbsp;" }}/>);
    }
    return (<div className={className} style={style} data-item-view-type={item.type}>
      {body}
    </div>);
}
