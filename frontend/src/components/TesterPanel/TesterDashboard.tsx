...
import { fetchLogs, fetchTesterRun, fetchTesterRuns, fetchTrace, runTesterScript, type RunsSummary, type TestRunRecord, } from "../../lib/observability";
...
export default function TesterDashboard() {
    ...
    async function handleRun(e: FormEvent) {
        ...
        try {
            ...
            setMessage(`Run ${result.runId.slice(0, 8)}… ${result.passed ? "PASSED" : "FAILED"} in ${result.durationMs}ms`);
            await refresh();
    ...
    function exportReport() {
        const blob = new Blob([JSON.stringify({ summary, selectedRun, trace, relatedLogs }, null, 2)], { type: "application/json" });
    ...
    return (<div className="obs-dashboard tester-dashboard">
      ...
      {summary?.latestBuild && (<section className="panel tester-build-panel">
          ...
          <dl className="tester-detail__meta">
            ...
            <dt>Result</dt>
            <dd>
              <span className={`obs-badge obs-badge--${summary.latestBuild.passed ? "success" : "error"}`}>
          ...
          <ul className="tester-steps">
            {summary.latestBuild.steps.map((step, i) => (<li key={i}>
      ...
      {summary && (<section className="obs-dashboard__stats">
      ...
      <form className="panel tester-run-form" onSubmit={handleRun}>
        ...
        <div className={`form-field ${scriptError ? "form-field--error" : ""}`}>
          ...
          <input id="tester-script" value={script} onChange={(e) => setScript(e.target.value)} placeholder="smoke, integration, auth_flow…"/>
      ...
      <section className="obs-dashboard__grid">
        <div className="panel obs-table-wrap">
          ...
          <div className="obs-table-scroll">
            <table className="obs-table">
              ...
              <tbody>
                {summary?.runs?.map((run) => (<tr key={run.runId} className={selectedRun?.runId === run.runId
                ? "obs-table__row--active"
                : ""} onClick={() => selectRun(run.runId)}>
                    <td>{new Date(run.startedAt).toLocaleString()}</td>
                    <td>{run.script}</td>
                    <td>
                      <span className={`obs-badge obs-badge--${run.passed ? "success" : "error"}`}>
                ...
                {(!summary || (summary.runs?.length ?? 0) === 0) && (<tr>
        ...
        <div className="panel tester-detail">
          <h2>Run analysis</h2>
          {!selectedRun ? (<p className="page-lead">Select a run to inspect steps and traces.</p>) : (<>
              <dl className="tester-detail__meta">
                <dt>Run ID</dt>
              ...
              <h3>Steps</h3>
              <ul className="tester-steps">
                {selectedRun.steps.map((step, i) => (<li key={i}>
                    <span className={`obs-badge obs-badge--${step.status}`}>
                      {step.status}
                    </span>
                    <span>{step.name}</span>
              ...
              <ol className="obs-timeline">
                {trace.map((hop, i) => (<li key={i} className="obs-timeline__item">
              ...
              {relatedLogs.length > 0 && (<>
                  ...
                  <ul className="tester-related-logs">
                    {relatedLogs.map((log, i) => (<li key={i}>
