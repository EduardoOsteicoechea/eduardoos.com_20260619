...
export default function AuthForm({ mode }: AuthFormProps) {
    ...
    const [step, setStep] = useState<FormStep>(needsOtpStep(mode) ? "otp" : initialUrl.step);
    ...
    async function handleSubmit(event: FormEvent) {
        ...
        try {
            ...
            if (mode === "register") {
                ...
                if (!hasIssuedToken(result)) {
                    goToOtpStep(log.correlationId, "We emailed you a 6-digit code. Enter it below to finish creating your account.");
                    return;
                }
                finishAuth(result.message, log.correlationId);
                return;
            ...
            if (!result) {
                ...
                if (apiMessage.toLowerCase().includes("not verified")) {
                    goToOtpStep(log.correlationId, "Your email is not verified yet. Enter the 6-digit code we sent when you registered.");
                    return;
                }
    ...
    return (<form className="auth-form panel" onSubmit={handleSubmit}>
      ...
      <div className={`form-field ${fieldErrors.email ? "form-field--error" : ""}`}>
        ...
        <input id="auth-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" readOnly={showOtpField}/>
      ...
      {showOtpField && (<div className={`form-field auth-form__otp-field ${fieldErrors.otp ? "form-field--error" : ""}`}>
          ...
          <input id="auth-otp" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))} autoComplete="one-time-code" placeholder="123456" autoFocus/>
      ...
      <div className="panel__actions">
        ...
        {showOtpField && mode !== "verify-otp" && (<button type="button" className="btn" disabled={loading} onClick={() => {
                clearOtpStepFromUrl();
                setStep("credentials");
                setOtp("");
                setMessage("");
