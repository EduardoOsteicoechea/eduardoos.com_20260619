...
function SeekBar({ canPlay, currentTime, duration, onSeek, onSeekStart, onSeekEnd, }: Pick<PlaylistControlsProps, "canPlay" | "currentTime" | "duration" | "onSeek" | "onSeekStart" | "onSeekEnd">) {
    ...
    return (<div className="playlist-controls__progress">
      <span className="playlist-controls__time" aria-hidden="true">
        {formatPlaybackTime(currentTime)}
      </span>
      <div className="playlist-controls__seek-wrap">
        <div className="playlist-controls__seek-fill" style={{ width: `${progressPercent}%` }} aria-hidden="true"/>
        <input className="playlist-controls__seek" type="range" min={0} max={maxDuration} step={0.1} value={seekValue} disabled={!canPlay || maxDuration <= 0} aria-label="Playback position" aria-valuetext={`${formatPlaybackTime(currentTime)} of ${formatPlaybackTime(duration)}`} onPointerDown={onSeekStart} onChange={(e) => onSeek(Number(e.target.value))} onPointerUp={(e) => onSeekEnd(Number((e.target as HTMLInputElement).value))} onKeyUp={(e) => {
...
function MetaPanel({ nowPlayingLabel, volume, playbackRate, volumeId, speedId, onVolumeChange, onSpeedChange, }: MetaPanelProps) {
    return (<>
      ...
      <div className="playlist-controls__group">
        ...
        <input id={volumeId} className="playlist-controls__slider" type="range" min={0} max={1} step={0.01} value={volume} title="Volume" onChange={(e) => onVolumeChange(Number(e.target.value))}/>
      </div>
      <div className="playlist-controls__group">
        <label className="playlist-controls__label" htmlFor={speedId} title="Playback speed">
          Speed
        </label>
        <select id={speedId} className="playlist-controls__select" value={playbackRate} title="Playback speed" onChange={(e) => onSpeedChange(Number(e.target.value))}>
          {SPEED_OPTIONS.map((rate) => (<option key={rate} value={rate}>
              {rate}x
            </option>))}
        </select>
      </div>
      ...
      <div className="playlist-controls__inner">
        ...
        <div className="playlist-controls__deck">
          <div className="playlist-controls__transport">
            <button type="button" className="playlist-controls__btn" disabled={!canPlay} title="Previous track" aria-label="Previous track" onClick={onPrevious}>
              <IconPrevious />
            </button>
            <button type="button" className="playlist-controls__btn playlist-controls__btn--primary" disabled={!canPlay} title={isPlaying ? "Pause" : "Play"} aria-label={isPlaying ? "Pause" : "Play"} onClick={isPlaying ? onPause : onPlay}>
              {isPlaying ? <IconPause /> : <IconPlay />}
            </button>
            <button type="button" className="playlist-controls__btn" disabled={!canPlay} title="Stop" aria-label="Stop" onClick={onStop}>
              <IconStop />
            </button>
            <button type="button" className="playlist-controls__btn" disabled={!canPlay} title="Next track" aria-label="Next track" onClick={onNext}>
              <IconNext />
            </button>
          ...
          <button ref={trayToggleRef} type="button" className={`playlist-controls__btn playlist-controls__tray-toggle${trayOpen ? " playlist-controls__tray-toggle--open" : ""}`} title="Track info, volume, and speed" aria-label="Track info, volume, and speed" aria-expanded={trayOpen} aria-controls="playlist-controls-tray" onClick={() => setTrayOpen((open) => !open)}>
            <IconMixer />
          </button>
        ...
        <div ref={trayRef} id="playlist-controls-tray" className={`playlist-controls__tray${trayOpen ? " playlist-controls__tray--open" : ""}`} role="region" aria-label="Track info and playback settings" hidden={!trayOpen}>
