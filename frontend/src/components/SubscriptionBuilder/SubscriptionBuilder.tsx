...
import { createSubscriptionIntent, fetchEntitlementsForEmail, fetchMyEntitlements, quoteSubscription, SUBSCRIPTION_SERVICES, type BillingPeriod, type EntitlementRecord, type PaymentIntentResponse, } from "../../lib/subscriptions";
...
export default function SubscriptionBuilder() {
    ...
    const activeServiceIds = useMemo(() => new Set(entitlements.map((record) => record.service_id)), [entitlements]);
    ...
    const total = useMemo(() => quoteSubscription(checkoutSelected, billingPeriod), [checkoutSelected, billingPeriod]);
    useEffect(() => {
    ...
    async function handlePrepare(event: FormEvent) {
        ...
        if (checkoutSelected.length === 0) {
            setError(activeServiceIds.size > 0 && billingPeriod === "monthly"
        ...
        setIntent(null);
        try {
            const { data, error: apiError } = await createSubscriptionIntent(email, checkoutSelected, billingPeriod);
            if (!data) {
            ...
            setIntent(data);
            setMessage(`Intent ${data.intent_id} ready for ${data.product_name}. Total: $${data.amount} ${data.currency}.`);
        }
        catch {
    ...
    const allServicesActiveMonthly = billingPeriod === "monthly" &&
        SUBSCRIPTION_SERVICES.every((service) => activeServiceIds.has(service.id));
    return (<div className="subscription-builder panel">
      ...
      <div className="subscription-builder__pricing-toggle" role="group" aria-label="Billing period">
        <button type="button" className={`btn ${billingPeriod === "monthly" ? "btn--primary" : ""}`} onClick={() => {
            setBillingPeriod("monthly");
            setIntent(null);
        }}>
          Monthly ($1 / service)
        </button>
        <button type="button" className={`btn ${billingPeriod === "yearly" ? "btn--primary" : ""}`} onClick={() => {
      ...
      <div className="subscription-builder__services">
        {SUBSCRIPTION_SERVICES.map((service) => {
            ...
            return (<label key={service.id} className={`subscription-builder__service ${checked ? "subscription-builder__service--selected" : ""}${isBlocked ? " subscription-builder__service--active" : ""}${isExtendable ? " subscription-builder__service--extendable" : ""}`}>
              <input type="checkbox" checked={checked} disabled={isBlocked} onChange={() => toggleService(service.id)}/>
              <span className="subscription-builder__service-copy">
                <strong>{service.label}</strong>
                <span>{service.description}</span>
                {isExtendable ? (<span className="subscription-builder__service-status">
                    Add 1 year — active until {new Date(active!.valid_until).toLocaleString()}
                  </span>) : isActive ? (<span className="subscription-builder__service-status">
                    Subscribed until {new Date(active!.valid_until).toLocaleString()}
                  </span>) : (<span className="subscription-builder__service-price">${unit}/period</span>)}
              </span>
            </label>);
        })}
      </div>
      ...
      <form className="subscription-builder__prepare" onSubmit={handlePrepare}>
        <div className={`form-field ${emailError ? "form-field--error" : ""}`}>
          ...
          <input id="subscription-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" required/>
          {emailError && <span className="field-error">{emailError}</span>}
        </div>
        <div className="panel__actions">
          <button className="btn btn--primary" type="submit" disabled={loading || checkoutSelected.length === 0 || allServicesActiveMonthly}>
            {loading ? "Preparing…" : "Prepare checkout"}
          </button>
          {intent && (<button className="btn" type="button" onClick={handleCheckStatus} disabled={loading}>
              Check status
            </button>)}
        </div>
      </form>
      ...
      {intent && (<form className="subscription-builder__checkout" action={intent.paypal_checkout_url || PAYPAL_FORM_ACTION} method="post" target="_top">
      ...
      {entitlements.length > 0 && (<section className="subscription-builder__active" aria-label="Active services">
          <h2>Your active services</h2>
          <ul>
            {entitlements.map((item) => (<li key={item.service_id}>
                <strong>{item.service_label}</strong> until {new Date(item.valid_until).toLocaleString()}
              </li>))}
          </ul>
        </section>)}

      {!isAuthenticated() && (<p className="subscription-builder__hint">
