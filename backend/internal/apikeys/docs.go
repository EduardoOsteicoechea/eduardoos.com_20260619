	...
	return DocsCatalog{
		...
		Routes: []DocsRoute{
			...
			},
			{
				...
				Auth:         "api_key",
				Summary:      "Step 2 — list owned reports available to edit.",
				Requirements: "Owned reports only (id, tema, reportNumber, updatedAt).",
			},
			{
				Method:       http.MethodGet,
				Path:         "/api/v1/ereport/reports/{ownerSafe}/{reportId}",
				Auth:         "api_key",
				Summary:      "Step 3a — read one owned eReport (meta + full payload).",
				Requirements: "ownerSafe must be the key owner's safe id; report must be owned by that user.",
			},
			{
				Method:       http.MethodPost,
				Path:         "/api/v1/ereport/reports/{ownerSafe}/{reportId}",
				Auth:         "api_key",
				Summary:      "Step 3b — full-replace report payload after snapshotting the previous version.",
				Body:         `{"confirmOverwrite":true,"tema":"optional","payload":{ /* full .ereport JSON */ }}`,
				Requirements: "confirmOverwrite must be JSON true. payload required. Prior version saved to history (max 50).",
			},
		},
		KeyMgmt: []DocsRoute{
			{
