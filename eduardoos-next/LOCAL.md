...
3. `/media/musica` — worship library + session playlist + lyrics; timed lyric editor for `eduardooost@gmail.com`. **Admin-only mic recorder** (`SongRecorder`) → `POST /api/media/audio/upload` → S3 `media/worship_playlists/` → track lands in the playlist and `.emusic` shell is ensured for lyrics edit.
