...
- **Admin users**: `/admin/users` (platform admin eduardooost@gmail.com only — JWT + IsAdminEmail on `/api/admin/*`). Access|Subscriptions editor + delete with confirm. **Bulk register**: paste/upload JSON array (`name`/`nombre`, `email`/`correo`, `password`/`contrasena`) → `POST /api/admin/users/bulk-register` creates unverified accounts in the same user store as public register and sends OTP confirmation email per row via shared SMTP; returns per-row created/failed summary (max 100). Delete/entitlements resolve email via `?email=` (preferred) or URL-decoded path `{email}` so spam accounts with many dots (e.g. `b.ero…@gmail.com`) can be removed; register-only `IsSpammyLocalPart` is not applied on admin mutate/bulk. Static HTML published atomically. API failures use ServerErrorModal / openApiErrorModal.
...
- **Wired**: pamphlet visual generator (`src/lib/pamphlet-generator`, mount on `/documents/pamphlet` with cloud EPAMs via `/api/epams`; Print downloads full Roboto landscape PDF via `POST /api/documents/pamphlet/pdf`), **Music** worship builder (`PlaylistBuilder` + `/api/media/audio` + admin-only `POST /api/media/audio/upload` mic recorder → S3 `worship_playlists` → session playlist + `/api/emusic` lyric shell; lyric structure editor with `--site-*` theme + multi-select bulk delete for APS admin), subscription intents + PayPal hosted button placeholder, OpenBIM upload/list/download + **That Open / web-ifc 3D viewer** on `/bim` (memory; optional `IFCBIM_S3_BUCKET`; WASM at `/web-ifc/`; Fragments camera-update sync so geometry appears after fit), edebat list/create/turn (memory), **The Instrumentalist** (`/instrumentalist`: chat + topic by default; Beliefs header-menu toggle for `@xyflow/react` tree; single default `.instru` upsert; JWT `/api/instrumentalist*`; DeepSeek analyze/chat when `DEEPSEEK_API_KEY` set; belief canvas uses `deleteKeyCode={null}` + canvas-scoped Backspace/Delete so toolbar/input focus cannot wipe selected ideas), **Homescool** (`/homescool`: register students, teacher roster/workspace, student `/homescool/learning`; JWT `/api/homescool/*`; durable DynamoDB links + task templates/assignments in `eduardoos_catalog`; S3 `homeschool/{teacher}/{student}/…` + submissions; teacher catalogs = Period + Study area only; templates/tasks store **`studyAreas: string[]`** (legacy `studyArea` string migrates to a one-item array; UI checkbox multi-picker); assignment **`frequency`** (`once` / `daily` / `daily_except` + exclude weekdays; boards = one card, Calendar expands occurrences); **Calendar** sidebar via FullCalendar v6 month/week (`--site-*` theme); task duration = fixed Spanish presets stored as `durationMin` (`30m`/`1h`/`2d`/`1w`/`3mo`…); four teacher boards stacked full-width + student pending board; folders header toggle = sidebar-rail icon; aside symmetric padding; grade 1–5 segment bar; atelier-styled HTML emails via shared `SMTP_USER`/`SMTP_PASS` for register / assign / grade; checks `go test ./internal/homescool/...`, `npm run test:homescool`; spec `specs/003-homescool-calendar/`), **Greek** (admin-only `/greek` + `/greek/build` + `/greek/build/{grupo}`; JWT `/api/greek/*` + `/api/greek/catalog` (+ gallery aliases); **clean letter catalog** = 49 slots: 24 upper + 24 lower Αα…Ωω + final ς; alphabet # `n` / `n.1` / `18.2`; re-seed refreshes clean set, prunes obsolete undrawn accent orphans, keeps drawn SVGs for remaining slots; under S3 `greek/{user}/gallery/`; catalog rows show SVG preview + trash (`confirm` → `DELETE /api/greek/catalog/{slug}` clears drawing, keeps seed slot); words pick catalog glyphs only; draw/edit pad displays 128×256 → exports 32×64 SVG; DynamoDB groups + S3 `greek/{user}/{group}/`; checks `go test ./internal/greek/...`, `npm run test:greek`; IAM `greek/*`), **Church** (`/church` grid+search, `/church/register`, `/church/overview` + FullCalendar v6, `/church/activity` reports, `/church/{denom}/{id}` tabs; JWT `/api/church/*`; membership roles `church-admin`/`church-member` in Dynamo + S3 `church/{denom}/{id}/`; platform admin full access; checks `go test ./internal/church/...`, `npm run test:church`; IAM `church/*`; spec `specs/004-church/`)
