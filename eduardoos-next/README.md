...
- **Chrome**: `Header` (Home, Contact, OpenBIM, APS, Services dropdown, **Theme** light/dark toggle; signed-in **avatar** always in the bar next to Menu â high-contrast `--site-accent` / `--site-accent-fg` initial with surface ring + shadow; optional photo via `src/lib/profile.ts` â `GET /api/auth/profile`), `AuthGate`, `BaseLayout` / `PamphletLayout`
- **Design**: blueprint / steel tokens in `src/styles/theme.css`; agent skills `.cursor/skills/frontend-design`, `.cursor/skills/bim-aec-frontend`, and `.cursor/skills/agent-voice` (identity + tone; also in `specs/001-platform-parity/spec.md`). Activity bar icons use `currentColor` / `--site-body-fg` / `--site-accent-fg` (legible in light and dark).
