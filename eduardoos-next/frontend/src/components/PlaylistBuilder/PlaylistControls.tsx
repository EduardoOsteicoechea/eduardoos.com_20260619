  ...
  return (
    <>
      ...
      <div className="playlist-controls__group">
        <label
          className="playlist-controls__label playlist-controls__label--icon"
          htmlFor={volumeId}
          title="Volume"
        >
          <IconVolume />
...
export default function PlaylistControls({
  ...
  onSeekEnd,
  onLoopToggle,
  emusicsDownloading = false,
  emusicsProgress = "",
  emusicsReadyCount = 0,
  emusicsLibraryCount = 0,
  onDownloadEmusics,
  onLoadEmusics,
}: PlaylistControlsProps) {
  ...
  function nudgeSeek(deltaSeconds: number) {
    if (!canPlay) return;
    const max = Number.isFinite(duration) && duration > 0 ? duration : Number.POSITIVE_INFINITY;
    const next = Math.max(0, Math.min(max, currentTime + deltaSeconds));
    onSeek(next);
    onSeekEnd(next);
  }
